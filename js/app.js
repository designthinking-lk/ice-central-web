...
(function () {
  ...
  function renderChrome() {
    ...
    // Data export (photos + LLM personas as a zip) — mentors and admins only.
    var navData = $('#navData');
    if (navData) navData.hidden = !(d.isAdmin || (d.me && hasRoleU(d.me, 'mentor')));
  ...
  function viewLanding() {
    return '<div class="landing">' +
      ...
      '<source src="assets/video/ice2026-landing.mp4?v=3" type="video/mp4"></video>' +
      '</div>' +
  ...
  function dataSlug(name) {
    var s = String(name || 'person').trim();
    try { s = s.replace(/[^\p{L}\p{N}\s_-]/gu, ''); }   // drop punctuation/emoji
  ...
  // hash, so repeat exports are cheap). Empty string if disabled/unavailable.
  async function dataFetchPersona(u) {
    try {
      var r = await A.api('persona', {
        name: u.name || '',
        role: hasRoleU(u, 'mentor') ? 'mentor' : hasRoleU(u, 'catalyst') ? 'catalyst' : 'participant',
        affiliation: u.affiliation || '',
  ...
  var dataExportBusy = false;
  async function exportPeopleData(btn) {
    if (dataExportBusy) return;
    var users = ((state.data && state.data.users) || []).filter(function (u) { return u && u.name; });
  ...
  function viewSystemMap() {
    ...
    return '' +
      '<div class="smap" id="systemMap">' +
        '<div class="aurora smap-aurora" aria-hidden="true"><span></span><span></span><span></span></div>' +
  ...
  function initSystemMap() {
    ...
    var NODES = [
      { id:'client', x:8,  y:53, icon:'globe',   name:'Visitor',        tag:'Browser · PWA',        stat:'online',        statLabel:'online',  desc:'The participant’s browser. Installable PWA; talks only to the edge and the two Apps Script endpoints.' },
      { id:'edge',   x:28, y:44, icon:'cloud',   name:'Cloudflare Worker', tag:'routing · /vid · OG', desc:'Front door. Routes each project subdomain, proxies Drive video through /vid, and renders share-card images.' },
      ...
      { id:'auth',   x:50, y:25, icon:'key',     name:'Auth',           tag:'Apps Script · OAuth',  desc:'Separate Apps Script web app. Runs Google sign-in and hands the SPA a verified identity token.' },
      { id:'api',    x:50, y:53, icon:'server',  name:'Central API',    tag:'Apps Script · Code.js', desc:'The core. Every read/write, provisioning, wallet and invite action flows through this one deployment.' },
      { id:'applefn',x:50, y:81, icon:'wallet',  name:'Apple Wallet fn', tag:'Node function',       desc:'Node function that signs .pkpass bundles for Apple Wallet, invoked by the Central API on score updates.' },
      { id:'sheets', x:74, y:21, icon:'grid',    name:'Sheets',         tag:'registry · project DB', stat:'members',      statLabel:'members', desc:'The database. A central registry plus one sheet per project holds users, teams, projects and invites.' },
      { id:'drive',  x:74, y:38, icon:'folder',  name:'Drive',          tag:'uploads · video',      stat:'videos',        statLabel:'videos',  desc:'Stores profile media and 1080p intro/pitch videos; streamed back to the browser via the edge /vid proxy.' },
      { id:'admin',  x:74, y:55, icon:'users',   name:'Admin SDK',      tag:'@designthinking.lk',   desc:'Mints firstname@designthinking.lk accounts into the ICE org unit when a registration completes.' },
      { id:'gmail',  x:74, y:72, icon:'mail',    name:'Gmail',          tag:'invites · passwords',  stat:'invitesPending', statLabel:'pending', desc:'Sends branded invites, onboarding notes and temporary passwords from the verified send-as alias.' },
      { id:'cal',    x:74, y:89, icon:'calendar',name:'Calendar',       tag:'program events',       desc:'Backs the Program view — session times and colour-coded tracks, kept in sync by the Central API.' },
      { id:'gwallet',x:93, y:39, icon:'wallet',  name:'Google Wallet',  tag:'passes',               stat:'walletPushes',  statLabel:'pushes',  desc:'Google Wallet API. The Central API pushes the live member pass and updates scores on it.' },
      { id:'ghorg',  x:93, y:69, icon:'github',  name:'GitHub org',     tag:'auto-invite',          stat:'githubInvited', statLabel:'invited', desc:'Registrants are auto-invited to the designthinking-lk GitHub org once their profile is complete.' }
    ];
    var EDGES = [
  ...
  document.addEventListener('click', async function (e) {
    ...
    switch (action) {
      ...
      case 'download-data': exportPeopleData(t); break;
